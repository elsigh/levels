  ...
  def _get_params(self, validator_parameter, name_prefix):
    ...
    for key in self.request.arguments():
      if key.startswith(name_prefix):
        ...
        if key == 'mapper_params.filters' and self.request.get(key, None):
          filters = self.request.get(key)
          values = ast.literal_eval(filters)
          #values = [['means', '=', 'self_battery_message'],
          #          ['created', '>', datetime.datetime(2014, 10, 10)],
          #          ['created', '<', datetime.datetime.now()],
          #          ]

        logging.info('KEY: %s, V: %s' % (key, values))

        adjusted_key = key[len(name_prefix):]
        if len(values) == 1 and key != 'mapper_params.filters':
